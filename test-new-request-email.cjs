 ...
 * Folosește formatul CommonJS (.cjs) pentru a fi compatibil cu importurile
 */
...
async function main() {
  ...
  try {
    ...
    const requestTitle = 'Test Reparație Motor Fix';
    const clientName = 'Client Test Fix';
    const uniqueRequestId = `test_request_fix_${Date.now()}`;
