...
class NotificationHelper {
   ...
   */
  static testNotification(title: string = 'Test notificare', options: NotificationOptions = {}): void {
    console.log('Inițiere test notificări browser...');
    
    // Opțiuni implicite pentru notificarea de test
    const defaultOptions: NotificationOptions = {
    ...
    // Afișăm notificarea
    this.showNotification(title, mergedOptions);
  }
  
  /**
   * Pornește verificarea mesajelor în fundal utilizând Service Worker
   */
  static startBackgroundMessageCheck(userId: number, userRole: 'client' | 'service', token: string): Promise<any> {
    if (!this.isSupported() || this.checkPermission() !== 'granted') {
      console.warn('Notificările nu sunt suportate sau permise');
      return Promise.reject(new Error('Notificările nu sunt suportate sau permise'));
    }
    
    if (!userId || !userRole || !token) {
      console.error('Lipsesc informații necesare pentru verificarea în fundal');
      return Promise.reject(new Error('Lipsesc informații necesare pentru verificarea în fundal'));
    }
    
    if (!('serviceWorker' in navigator) || !navigator.serviceWorker.controller) {
      console.error('Service Worker nu este disponibil sau activ');
      return Promise.reject(new Error('Service Worker nu este disponibil sau activ'));
    }
   ...
   */
  static stopBackgroundMessageCheck(): Promise<any> {
    if (!('serviceWorker' in navigator) || !navigator.serviceWorker.controller) {
      console.warn('Service Worker nu este disponibil sau activ');
      return Promise.resolve(); // Returnăm un promise rezolvat pentru că nu avem ce opri
    }
  ...
  /**
   * Trimite un mesaj către Service Worker și așteaptă răspunsul
   */
  private static sendServiceWorkerMessage(message: any): Promise<any> {
    return new Promise((resolve, reject) => {
      try {
        if (!('serviceWorker' in navigator) || !navigator.serviceWorker.controller) {
          reject(new Error('Service Worker nu este disponibil sau activ'));
          return;
        }
        
        // Generăm un ID unic pentru acest mesaj pentru a putea asocia răspunsul
        const messageId = 'msg_' + new Date().getTime() + '_' + Math.random().toString(36).substr(2, 9);
        
        // Funcția care va fi apelată când primim răspunsul de la Service Worker
        const messageListener = (event: MessageEvent) => {
          if (event.data && event.data.id === messageId) {
            // Am primit răspunsul pentru acest mesaj
        ...
        // Înregistrăm handler-ul pentru răspuns
        ...
        // Setăm un timeout pentru cazul în care nu primim răspuns
        setTimeout(() => {
          navigator.serviceWorker.removeEventListener('message', messageListener);
          reject(new Error('Timeout la așteptarea răspunsului de la Service Worker'));
        }, 5000);
      } catch (error) {
        reject(error);
      }
    });
  }onst permission = this.checkPermission();
  ...
  /**
   * Verifică dacă trebuie să afișeze notificări pentru anumite evenimente
   * bazat pe preferințele utilizatorului și tipul de notificare
   */
  static async shouldShowNotification(userRole: string, notificationType: string): Promise<boolean> {
    try {
      // Verificăm mai întâi permisiunea browserului
      if (this.checkPermission() !== 'granted') {
        return false;
      }
      
      // Obținem preferințele de notificări
      const preferences = await this.getNotificationPreferences(userRole);
      
      // Verificăm dacă notificările browser sunt activate global
      if (!preferences.browserNotificationsEnabled) {
        return false;
      }
      
      // Verificăm pentru tipuri specifice de notificări
      switch (notificationType) {
        ...
        case 'offer':
          return userRole === 'client' ? 
            preferences.newOfferBrowserEnabled : 
            preferences.acceptedOfferBrowserEnabled;
        case 'request':
          return userRole === 'service' ? 
            preferences.newRequestBrowserEnabled : 
            true;
        case 'review':
          return userRole === 'service' ? 
            preferences.newReviewBrowserEnabled : 
            true;
        default:
          return true; // Implicit permitem alte tipuri de notificări
      }
    } catch (error) {
      console.error(`Eroare la verificarea preferințelor de notificări:`, error);
      return true; // Implicit permitem notificările în caz de eroare
    }
  }
