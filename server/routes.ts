...
export function registerRoutes(app: Express): Server {
  ...
  // Add endpoint to get working hours for a service provider
  app.get("/api/service/working-hours/:serviceProviderId", async (req, res) => {
    try {
      ...
      const workingHours = await db.query.workingHours.findMany({
        where: eq(schema.workingHours.serviceProviderId, serviceProviderId)
      });
  ...
  // Add endpoint to update working hours
  app.post("/api/service/working-hours", validateFirebaseToken, async (req, res) => {
    try {
      ...
      if (!provider) {
        return res.status(403).json({ error: "Access denied. Only service providers can update working hours." });
      }

      const { day, openTime, closeTime, isClosed } = req.body;
      
      if (!day || (!isClosed && (!openTime || !closeTime))) {
        return res.status(400).json({ error: "Missing required fields" });
      }

      // Check if working hours already exist for this day
      const existingHours = await db.query.workingHours.findFirst({
        where: and(
          eq(schema.workingHours.serviceProviderId, provider.id),
          eq(schema.workingHours.dayOfWeek, day)
        )
      });
      ...
      if (existingHours) {
        ...
        result = await db.update(schema.workingHours)
          .set({
            openTime,
            closeTime,
            isClosed
          })
          .where(eq(schema.workingHours.id, existingHours.id))
          .returning();
      } else {
        // Insert new hours
        result = await db.insert(schema.workingHours)
          .values({
  ...
  // Add endpoint to get reviews for a service provider
  app.get("/api/service/reviews/:serviceProviderId", async (req, res) => {
    try {
      const serviceProviderId = parseInt(req.params.serviceProviderId);
      ...
      const reviews = await db.query.reviews.findMany({
        where: eq(schema.reviews.serviceProviderId, serviceProviderId),
  ...
  app.post("/api/client/reviews", validateFirebaseToken, async (req, res) => {
    try {
      ...
      if (!serviceProviderId || !rating || rating < 1 || rating > 5) {
        return res.status(400).json({ error: "Invalid review data" });
      }

      // Check if service provider exists
      const serviceProvider = await db.query.serviceProviders.findFirst({
        where: eq(schema.serviceProviders.id, serviceProviderId)
      });
      ...
      // Check if client already submitted a review for this service provider
      const existingReview = await db.query.reviews.findFirst({
        where: and(
          eq(schema.reviews.clientId, client.id),
          eq(schema.reviews.serviceProviderId, serviceProviderId)
        )
      });
      ...
      if (existingReview) {
        ...
        result = await db.update(schema.reviews)
          .set({
            rating,
            comment
          })
        ...
        // Insert new review
        result = await db.insert(schema.reviews)
          .values({
            clientId: client.id,
            serviceProviderId,
            rating,
