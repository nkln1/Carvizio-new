...
export function registerRoutes(app: Express): Server {
  ...
  // Add client message send endpoint
  app.post("/api/client/messages/send", validateFirebaseToken, async (req, res) => {
    try {
      const { content, receiverId, requestId, offerId, receiverRole } = req.body;
      ...
      // Verify that client owns this request
      if (request.clientId !== client.id) {
        return res.status(403).json({ error: "Not authorized to send message for this request" });
      }

      // Get receiver (service provider) information
      ...
      const enrichedMessage = {
        ...
        receiverName: receiver.companyName
      };
