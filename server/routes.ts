...
// Handle test email endpoint for clients
app.post('/api/client/test-email', authenticateUser, async (req, res) => {
  try {
    ...
    const result = await EmailService.sendEmail(
      email,
      `Test notificări email [${testEmailId}]`,
      `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #e2e8f0; border-radius: 8px; overflow: hidden;">
        <div style="background-color: #3b82f6; padding: 20px; text-align: center;">
          <h2 style="color: white; margin: 0;">Test notificări email</h2>
        </div>
        <div style="padding: 20px;">
          <p style="font-size: 16px;">Bună ziua, ${name},</p>
          <p style="font-size: 16px;">Acesta este un email de test pentru a verifica că notificările prin email funcționează corect.</p>
          <div style="background-color: #f7fafc; border-left: 4px solid #3b82f6; padding: 15px; margin: 20px 0; border-radius: 4px;">
            <p style="margin: 0;">Dacă primiți acest email, înseamnă că notificările prin email sunt configurate corect.</p>
          </div>
          <p style="font-size: 16px;">Puteți continua să folosiți aplicația cu încredere că veți primi notificări pentru mesaje noi și oferte.</p>
          <div style="text-align: center; margin: 25px 0;">
            <a href="https://auto-service-app.replit.app/client-dashboard" 
          ...
          <div style="background-color: #f9f9f9; padding: 15px; border-radius: 4px; margin-top: 20px;">
            <p style="color: #718096; font-size: 14px; margin-top: 0; margin-bottom: 5px;">
              Acest email a fost trimis automat la cererea dvs. pentru testarea sistemului de notificări.
            </p>
          </div>
        </div>
        <div style="background-color: #f1f5f9; padding: 15px; text-align: center; font-size: 12px; color: #64748b;">
          <p style="margin: 0;">© ${new Date().getFullYear()} Auto Service App. Toate drepturile rezervate.</p>
          <!-- Test Email ID: ${testEmailId} -->
        </div>
      </div>
      `,
      `Bună ziua, ${name},\n\nAcesta este un email de test pentru a verifica că notificările prin email funcționează corect.\n\nDacă primiți acest email, înseamnă că notificările prin email sunt configurate corect.\n\nPuteți continua să folosiți aplicația cu încredere că veți primi notificări pentru mesaje noi și oferte.\n\nAccesează dashboard-ul: https://auto-service-app.replit.app/client-dashboard\n\nAcest email a fost trimis automat la cererea dvs. pentru testarea sistemului de notificări.\n\n© ${new Date().getFullYear()} Auto Service App. Toate drepturile rezervate.`,
      `[Test Email] ${testEmailId}`
    );
    ...
    if (result) {
      ...
      res.status(500).json({
        success: false,
        message: 'Nu s-a putut trimite email-ul de test'
      });
    }
  } catch (error) {
